...
import pyutilities.utils.strings as strings
...
def init_logger(logger_name: str, add_null_handler: bool = True) -> logging.Logger:
    ...
    if not strings.is_str_empty(logger_name):  # init logger if name isn't empty
        ...
        if add_null_handler:
            tmp_log.addHandler(
                logging.NullHandler()
            )  # added dummy handler in order to avoid errors like 'no handlers'
            print("++++++++")
        return tmp_log
...
def setup_logging(
    default_path="configs/logging.yml", default_level=logging.INFO, env_key="LOG_CFG", logger_name=None
):
    """
